...
A more recent example of string reuse is in Breakfast in Tijuana. The string for searching the evidence is reused from scanning the evidence in Hoxton Breakout, so I had to change the line from "Scan Evidence" to "Search Evidence" just so it makes sense BiT. If I could I'd have more flavourful text, but Overkill's reuse of strings is fairly limiting.
